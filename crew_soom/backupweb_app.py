...
class FloodWebApp:
    ...
    def simulate_data_update(self):
        ...
        if self.data is not None and len(self.data) > 0:
            ...
            print(f"🔄 데이터 업데이트: {new_date} (총 {len(self.data)}행)")
    ...
    def real_data_update(self):
        ...
        try:
            ...
            if current_weather:
                ...
                print(f"🌤️  실제 데이터 업데이트: {new_row['obs_date']} (강수량: {new_row['precipitation']:.1f}mm)")
            else:
                print("❌ API 호출 실패 - 시뮬레이션 데이터로 대체")
                self.simulate_data_update()
    ...
    def process_weather_data(self, weather_data):
        ...
        try:
            ...
            precipitation = rain.get('1h', 0)  # 1시간 강수량
            if precipitation == 0:
                precipitation = rain.get('3h', 0) / 3  # 3시간 강수량을 시간당으로 변환
            ...
            new_row = {
                ...
                'month': now.month,
                'precip_ma3': precipitation,  # 단순화
                'precip_ma7': precipitation,  # 단순화
                ...
                'visibility': weather_data.get('visibility', 10000) / 1000  # km 단위
            }
            ...
            try:
                ...
                if self.api_available:
                    try:
                        ...
                        return jsonify({
                            'success': True,
                            'message': f'API 오류로 시뮬레이션 데이터 {added_count}개가 추가되었습니다.',
                            'old_count': old_count,
                            'new_count': new_count,
                            'added_count': added_count,
                            'latest_date': self.data_end_date.isoformat() if self.data_end_date else None,
                    ...
                    return jsonify({
                        'success': True,
                        'message': f'API 키가 없어 시뮬레이션 데이터 {added_count}개가 추가되었습니다.',
                        'old_count': old_count,
                        'new_count': new_count,
                        'added_count': added_count,
                        'latest_date': self.data_end_date.isoformat() if self.data_end_date else None,
                        'data_source': 'Simulation (No API Key)'
                    })
        ...
        <body>
            ...
            <div class="container">
                <div class="header">
                    ...
                    <p>실시간 데이터 업데이트 + 자동 모니터링 + 완전한 웹 기반 머신러닝 플랫폼</p>
            ...
            <script>
                ...
                async function updateData() {
                    showGlobalLoading('새로운 데이터를 가져오고 있습니다...');
                    try {
